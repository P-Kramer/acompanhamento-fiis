def pagina_FIIs2():
    ...
    if st.button("🚀 Analisar Selecionados") and selecionados:
        with st.spinner("🔄 Coletando dados..."):
            ...
            historico_path = r"C:\Users\User\Documents\OneDrive\Documentos\Guilherme\Códigos\Longview_FIIs\acompanhamento-fiis\Ferramenta\FIIs\historico_dividendos.csv"
            if os.path.exists(historico_path):
                df_anterior = pd.read_csv(historico_path)
            else:
                df_anterior = pd.DataFrame(columns=["Fundo", "Anterior Data-Base", "Anterior Pagamento", "Anterior Dividendo (R$)", "Data-Base", "Data Pagamento", "Último Dividendo (R$)"])

            def comparar(row):
                anterior = df_anterior[df_anterior["Fundo"] == row["Fundo"]]
                if anterior["Último Dividendo (R$)"].values[0] == None:
                    return "NOVO"
